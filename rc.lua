...
-- {{{ Tags
tags = {
   names = { "term", "web", "files", "vm", "other" },
   layout = { layouts[3], layouts[1], layouts[3], layouts[5], layouts[1] }
}
...
-- {{{ Freedesktop Menu 表示鼠标右击出现的菜单，这边相当于modkey + w
...
memwidget = lain.widgets.mem({
    settings = function()
        widget:set_markup(markup("#eca4c4", mem_now.used .. "M "))
    end
})
...
for s = 1, screen.count() do
    ...
    --[[ 程序运行的标签显示位置
    -- Create the bottom wibox 程序运行的标签显示位置
...
root.buttons(awful.util.table.join(
    -- awful.button({ }, 3, function () mymainmenu:toggle() end), --将鼠标右击的功能关闭
    -- function (c) c:kill()  
...
globalkeys = awful.util.table.join(
    ...
    -- alt + tab 按键组合切换当前workspace下的程序窗口
    ...
    -- Widgets popups
    awful.key({ altkey,           }, "c",      function () lain.widgets.calendar:show(2) end),
    awful.key({ altkey,           }, "f",      function () fswidget.show(2) end),
    awful.key({ altkey,           }, "w",      function () myweather.show(2) end),
    ...
    awful.key({ "Control", altkey }, "a",      function () awful.util.spawn(screenshot) end),
    ...
    --[[awful.key({ modkey, "Control" }, "v",
                  function ()
                      if client.focus then
                          ...
                          if tag then
                              awful.client.movetotag(awful.util.spawn(VM), 4)
                          end
                     end
                  end),
    ]]   
